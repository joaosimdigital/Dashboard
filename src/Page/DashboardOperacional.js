...
export default function DashboardOperacional() {
  ...
  const [totalOrdemServicoConcluido, setTotalOrdemServicoConcluido] =
    useState(0);
  ...
  const metaHabilitacaoDoMes =
    metasMensais.find((m) => m.mes === mesAtual)?.meta || 0;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const responseiqr = await fetch(
          "http://38.224.145.3:3003/total-clientes-retrabalho-instalacao"
        );
        const dataiqr = await responseiqr.json();
        setTotalIQR(dataiqr.porcentagem_retrabalho); // Armazenando o total de cadastros de SC

        const responseirr = await fetch(
          "http://38.224.145.3:3003/clientes-repetidos-30-dias"
        );
        const datairr = await responseirr.json();
        setTotalIRR(datairr.porcentagem_repetidos); // Armazenando o total de cadastros de SC

        const responsehojehabilitados = await fetch(
          "http://38.224.145.3:3003/total-clientes-habilitados-hoje"
        );
        ...
        const responsemeshabilitadossc = await fetch(
          "http://38.224.145.3:3003/total-clientes-habilitados-sc"
        );
        const datahabilitadosmesSC = await responsemeshabilitadossc.json();
        setTotalHabilitadosMesSC(
          datahabilitadosmesSC.total_clientes_habilitados
        ); // Armazenando o total de cadastros de SC

        const responsemeshabilitadosrs = await fetch(
          "http://38.224.145.3:3003/total-clientes-habilitados-rs"
        );
      ...
      setTotalRetrabalhoManutencao(dataRetrabalhoManutencao.porcentagem_retrabalho);
        ...
        const responsemesexecutadosc = await fetch(
        ...
        const responsiqi = await fetch(
          "http://38.224.145.3:3003/porcentagem-clientes-repetidos"
        );
        const dataiqi = await responsiqi.json();
        setTotalIQI(dataiqi.porcentagem_restante); // Armazenando o total de cadastros de SC

        const responsemesexecutadohoje = await fetch(
          "http://38.224.145.3:3003/total-clientes-habilitados-executado-dia"
        );
        ...
        const responsemordemdeserviconarede = await fetch(
          "http://38.224.145.3:3003/ordens-servico-executado-na-rede"
        );
        const dataordemdeserviconarede =
          await responsemordemdeserviconarede.json();
        setTotalOrdemServiconaRede(
          dataordemdeserviconarede.ordens_servico[0].total_clientes_habilitados
        ); // Armazenando o total de cadastros de SC

        const responsemordemdeserviconapessoa = await fetch(
          "http://38.224.145.3:3003/ordens-servico-executado-na-pessoa"
        );
        const dataordemdeserviconapessoa =
          await responsemordemdeserviconapessoa.json();
        setTotalOrdemServiconaPessoa(
          dataordemdeserviconapessoa.ordens_servico[0]
            .total_clientes_habilitados
        ); // Armazenando o total de cadastros de SC

        const responsemordensservicopendente = await fetch(
          "http://38.224.145.3:3003/ordens-servico-pendente"
        );
        const dataordensservicopendente =
          await responsemordensservicopendente.json();
        setTotalOrdemServicoPendente(dataordensservicopendente.total_pendentes); // Armazenando o total de cadastros de SC

        const responsemordensservicoconcluido = await fetch(
          "http://38.224.145.3:3003/ordens-servico-concluido"
        );
        const dataordensservicoconcluido =
          await responsemordensservicoconcluido.json();
        setTotalOrdemServicoConcluido(
          dataordensservicoconcluido.total_pendentes
        ); // Armazenando o total de cadastros de SC

        const response = await fetch(
          "http://38.224.145.3:3003/total-clientes-habilitados-ultimos-3-meses"
        );
        const data = await response.json();

        // Criando dados formatados para o gráfico
        const formattedGraficoData = data.total_clientes_habilitados.map(
          (item) => ({
            month: `${item.mes}`,
  ...
  const mediaDiariaCalculada = Math.floor(
    metaHabilitacaoDoMes / diasUteisNoMes1
  );
            ...
            <div className="row-div-card-dados-principais">
              <div className="body-div-card-dados-principais-dados">
                ...
                <div className="div-card-dados-principais-dados">
                  <div className="card-dados-principais-dados">
                    ...
                    <h1 className="h2-card-dados-principais-dados">
                      {totalHabilitadosMesSC}
                    </h1>
                  </div>
                  <div className="card-dados-principais-dados">
                    <h1 className="h1-card-dados-principais-dados">RS</h1>
                    <h1 className="h2-card-dados-principais-dados">
                      {totalHabilitadosMesRS}
                    </h1>
                  </div>
                </div>
                ...
                >
                  ...
                  <h1 className="h2-meta-card-dados-principais">
                    {metaHabilitacaoDoMes}
                  </h1>
                </div>
              </div>
              ...
              <div className="body-div-card-dados-principais-dados">
                <div className="div-card-dados-principais-dados-diaria">
                  <h1 className="h1-div-card-dados-principais-dados-diaria">
                    Diária
                  </h1>
                  <h1 className="h2-div-card-dados-principais-dados-diaria">
                    {totalHabilitadosHoje}
                  </h1>
                </div>
                ...
                >
                  <h1 className="h1-meta-card-dados-principais">
                    Meta / Média
                  </h1>
                  <h1 className="h2-meta-card-dados-principais">
                    {mediaDiariaCalculada}
                  </h1>
                </div>
              </div>
        ...
        <div className="row-div-instalacao-grafico">
          <div className="body1-agenda-operacao">
            <div className="div-agenda-operacao">
              <h1 className="h1-tabela-agenda-dia">
                Instalações pro trimestre
              </h1>
            ...
            {/* <div className="div-agenda2-operacao">
              ...
              {regionsData.map((region, index) => (
                <div className="row-titulo-agenda-operacao">
                  <h1 className="h1-titulo-agenda-operacao" key={index}>
                    {region.name}
                  </h1>
