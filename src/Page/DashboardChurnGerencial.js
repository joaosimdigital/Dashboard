...
function DashboardChurnGerencial() {
  ...
  const [qtdOrdens, setQtdOrdens] = useState(0);
  const [clientesRepetidos6Meses, setClientesRepetidos6Meses] = useState([]);

  const [clientesRepetidos, setClientesRepetidos] = useState([]);
  const [clientesRepetidosAtendimento, setClientesRepetidosAtendimento] =
    useState([]);
  const [motivoSelecionado, setMotivoSelecionado] = useState(null);
  const [atendimentosUltimos6Meses, setAtendimentosUltimos6Meses] = useState(
    []
  );
  ...
  const [limiteMeta, setLimiteMeta] = useState({
    porcentagem_batida: 0,
    porcentagem_restante: 100,
  });
  ...
  function formatTempoNaBase(dias) {
    if (dias == null || isNaN(dias)) return 0;
  ...
  const fetchLimiteMeta = async () => {
    try {
      ...
      if (dataInicial)
        url += `&data_inicio=${encodeURIComponent(
          dataInicial.toISOString().split("T")[0]
        )}`;
      if (dataFinal)
        url += `&data_fim=${encodeURIComponent(
  ...
  const fetchAtendimentosUltimos6Meses = async () => {
    try {
      setLoading(true);
      const params = new URLSearchParams();
      ...
      if (response.ok) {
        setAtendimentosUltimos6Meses(data.atendimentos || []);
        console.log("✅ Atendimentos carregados:", data.atendimentos.length);
      } else {
        console.error(
          "Erro ao buscar atendimentos últimos 6 meses:",
          data.error
        );
      }
    } catch (error) {
      console.error("Erro ao buscar atendimentos últimos 6 meses:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchCancelamentosInstalacao = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const dadosFormatados = Object.entries(agrupados).map(
          ([mes, total]) => ({
            mes,
            total,
          })
      ...
      console.error(
        "Erro na requisição de cancelamentos por instalação:",
        error
      );
    }
  };
  ...
  const fetchClientesRepetidos6Meses = async () => {
    try {
      ...
      if (response.ok && data.clientes_repetidos) {
        setClientesRepetidos6Meses(data.clientes_repetidos); // ✅ novo state
      } else {
        console.error(
          "Erro na resposta de clientes repetidos (6 meses):",
          data.error
        );
      }
    } catch (error) {
      console.error("Erro ao buscar clientes repetidos (6 meses):", error);
    }
  };
  ...
  const fetchClientesRepetidosAtendimento = async () => {
    try {
      ...
      if (response.ok && data.clientes_repetidos_atendimento) {
        ...
        console.error(
          "Erro na resposta de atendimentos repetidos:",
          data.error
        );
      }
    } catch (error) {
  ...
  const fetchAll = async (showLoading = false) => {
    ...
    const tasks = [
      ...
      fetchStatusMeta,
      fetchAtendimentosUltimos6Meses,
 ...
 const handleUserInteraction = async (callback) => {
  ...
  return (
    ...
    >
      ...
      <div className="div-body-opcao">
        <div className="div-body-opcao1">
          ...
          <DatePicker
            ...
            onChange={(date) =>
              handleUserInteraction(() => setDataInicial(date))
            }
      ...
      <div className="row-gerencial-body1">
        <div className="div-gerencial-body1">
          <div className="grafico-div-gerencial-body1">
            <div className="grafico-card1-div-gerencial-body1">
              ...
              <h1 className="h2-card1-div-gerencial-body1">
                Indicador que mede o churn mensal.
              </h1>
                    ...
                    onClick={(data) =>
                      handleUserInteraction(() => {
                        if (data && data.activeLabel) {
                          const [mesTexto, anoCurto] =
                            data.activeLabel.split("/");
                          const mesesNomes = [
                            "Jan",
                            "Fev",
                            "Mar",
              ...
              <PieChart>
                <Pie
                  data={[
                    ...
                    {
                      name: "Restante",
                      value: limiteMeta.porcentagem_restante,
                    },
                  ]}
                  cx="50%"
                  cy="50%"
        ...
        <div className="div-gerencial-body1">
          <div className="card1-div-gerencial-body1">
            ...
            <h1 className="h2-div-gerencial-body1">
              {statusMeta.toLocaleUpperCase()}
            </h1>
          </div>

          <div
            className="card3-div-gerencial-body1"
            ...
            <tbody>
              {churnPorBairro
                .filter(
                  (item) =>
                    (!cidadeSelecionada ||
                      item.cidade_nome === cidadeSelecionada) &&
                    (!motivoSelecionado ||
                      cancelamentosDetalhados.some(
                        (cancel) =>
                ...
                .sort((a, b) =>
                  ordenarPorCancelamentos
                    ? b.total_cancelamentos - a.total_cancelamentos
                    : 0
                )
                .map((item, index) => (
                  <tr
                    ...
                    onClick={() =>
                      handleUserInteraction(() => {
                        setBairroSelecionado((prev) =>
                          prev === item.bairro_nome ? null : item.bairro_nome
                        );
                    ...
                    style={{
                      cursor: "pointer",
                      backgroundColor:
                        bairroSelecionado === item.bairro_nome
                          ? "#f2f2f2"
                          : "white",
                    }}
                  >
                    ...
                    <td className="h2-tabela-bairros-gerencial">
                      {item.total_cancelamentos}
                    </td>
                    <td className="h2-tabela-bairros-gerencial">
                      {item.total_clientes_ativos}
                    </td>
                    <td className="h2-tabela-bairros-gerencial">
                      {item.pct_pedido}%
                    </td>
                  ...
                  .filter(
                    (item) =>
                      (!bairroSelecionado ||
                        churnPorBairro.find(
                          (b) =>
                            b.bairro_nome === bairroSelecionado &&
                            b.cidade_nome === item.cidade_nome
                        )) &&
                      (!motivoSelecionado ||
                        cancelamentosDetalhados.some(
                          (cancel) =>
                            cancel.cidade_nome === item.cidade_nome &&
                            cancel.motivo_cancelamento === motivoSelecionado
                        ))
                  )
                  .map((item, index) => (
                    <tr
                      ...
                      onClick={() =>
                        handleUserInteraction(() => {
                          setCidadeSelecionada((prev) =>
                            prev === item.cidade_nome ? null : item.cidade_nome
                          );
                      ...
                      style={{
                        cursor: "pointer",
                        backgroundColor:
                          cidadeSelecionada === item.cidade_nome
                            ? "#f2f2f2"
                            : "white",
                      }}
                    >
                      ...
                      <td className="h2-tabela-bairros-gerencial">
                        {item.perc_cancelamento_pedido}%
                      </td>
                      <td className="h2-tabela-bairros-gerencial">
                        {item.perc_cancelamento_automatico}%
                      </td>
                      <td className="h2-tabela-bairros-gerencial">
                        {item.churn_rate}%
                      </td>
      ...
      <div className="div2-gerencial-body1">
        <div className="lista-div2-gerencial-body1">
          <h1 className="h1-div-gerencial-gerencial">
              ...
              {motivosCancelamentos
                .filter((item) => {
                  if (bairroSelecionado || cidadeSelecionada) {
                    return cancelamentosDetalhados.some(
                      (cancelamento) =>
                        (!bairroSelecionado ||
                          cancelamento.bairro === bairroSelecionado) &&
                        (!cidadeSelecionada ||
                          cancelamento.cidade_nome === cidadeSelecionada) &&
                ...
                .map((item, index) => (
                  <tr
                    ...
                    onClick={() =>
                      handleUserInteraction(() =>
                        setMotivoSelecionado((prev) =>
                          prev === item.descricao ? null : item.descricao
                        )
                      )
                    }
                    style={{
                      cursor: "pointer",
                      backgroundColor:
                        motivoSelecionado === item.descricao
                          ? "#f2f2f2"
                          : "white",
                    }}
                  >
      ...
      <div className="row-div-gerencial-body2">
        ...
        <div className="div-prejuizo-empresa">
          <div className="card-prejuizo-empresa">
            ...
            <h1 className="h2-div-gerencial-body1">
              {valorMensalCancelamento.toLocaleString("pt-BR", {
                style: "currency",
                currency: "BRL",
              })}
            </h1>
          ...
          <div className="card-prejuizo-empresa">
            ...
            <h1 className="h2-div-gerencial-body1">
              {valorAnualCancelamento.toLocaleString("pt-BR", {
                style: "currency",
                currency: "BRL",
              })}
            </h1>
                    ...
                    handleUserInteraction(() => {
                      if (data && data.activeLabel) {
                        ...
                        if (mesIndex !== -1) {
                          ...
                          const chaveAtual = mesSelecionadoGrafico
                            ? `${mesSelecionadoGrafico.mes}/${mesSelecionadoGrafico.ano}`
                            : null;
          ...
          <table className="tabela-bairros-gerencial">
            <thead>
              <tr style={{ backgroundColor: "white" }}>
                ...
                <th className="h1-tabela-bairros-gerencial">Bairro</th>
                <th className="h1-tabela-bairros-gerencial">Cidade</th>
                {/*<th className="h1-tabela-bairros-gerencial">Vendedor</th>{" "}
                {/* 🆕 */}
                <th className="h1-tabela-bairros-gerencial">
                ...
                <th className="h1-tabela-bairros-gerencial">Tipo</th>
                <th className="h1-tabela-bairros-gerencial">Tempo na Base</th>
                <th className="h1-tabela-bairros-gerencial" title="6 meses">
            ...
            <tbody>
              {cancelamentosDetalhados
                .filter((item) => {
                  ...
                  return (
                    (!motivoSelecionado ||
                      item.motivo_cancelamento === motivoSelecionado) &&
                    (!cidadeSelecionada ||
                      item.cidade_nome === cidadeSelecionada) &&
                    (!bairroSelecionado || item.bairro === bairroSelecionado) &&
                    (!filtroHabilitacaoMes ||
                      mesHab === filtroHabilitacaoMes) &&
                ...
                .map((item, index) => (
                  <tr key={index} className="link-tabela-bairros">
                    ...
                    <td className="h2-tabela-bairros-gerencial">
                      {item.tipo_pessoa.toUpperCase()}
                    </td>
                    <td className="h2-tabela-bairros-gerencial">
                      {formatTempoNaBase(item.tempo_na_base)}
                    </td>
                    <td className="h2-tabela-bairros-gerencial">
                    ...
                    <td className="h2-tabela-bairros-gerencial">
                      {new Date(item.data_cancelamento).toLocaleDateString(
                        "pt-BR"
                      )}
                    </td>
                    <td className="h2-tabela-bairros-gerencial">
      ...
      <div className="row-pre-churn">
              ...
              <tbody>
                {clientesRepetidos.map((item, index) => (
                  <tr className="link-tabela-bairros" key={index}>
                    <td
                      className="h2-tabela-bairros-gerencial1"
                      title={item.cliente}
                    >
                    ...
                    <td className="h2-tabela-bairros-gerencial1">
                      {item.qtd_ordens}
                    </td>
                  </tr>
                ))}
        ...
        <div className="card1-pre-churn">
          <div className="card-div-gerencial-body2">
            <h1 className="h1-div-gerencial-gerencial">
              Clientes com Atendimentos (3 meses)
            </h1>
            <table className="tabela-bairros-gerencial">
              <thead>
                <tr style={{ backgroundColor: "white" }}>
